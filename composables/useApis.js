    ...
    static async GetProducts(filter) {
      ...
      const data = ref(
        await new Promise((resolve) => {
          const products = ref([]);
          if (filter == "flash") {
            products.value = this.PRODUCTS.filter((product) => product.flash);
          } else if (filter === "topRated") {
            products.value = this.PRODUCTS.filter(
              (product) => product.topRated
            );
          } else if (filter?.categories_filter || filter?.rest_filters) {
            products.value = useHelpers().getSubRandomArr(this.PRODUCTS);
          } else {
            products.value = useHelpers().shuffle(this.PRODUCTS);
          }
    ...
    static async GetProduct(id) {
      ...
      const data = ref(
        await new Promise((resolve) => {
          ...
          product.value = this.PRODUCTS.find((item) => {
            return item.id == id;
          });
